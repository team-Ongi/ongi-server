...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<SignupResponse>> signup(@RequestBody @Valid SignupRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(response, SuccessStatus.SUCCESS_201));
    }
    ...
    @PostMapping("/senior/agreements")
    @Operation(summary = "로그인 후 서비스 이용 동의 요청")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "서비스 이용 동의 성공", content = @Content(mediaType = "application/json", schema = @Schema()))
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "모두 동의를 하지 않은 경우", content = @Content(mediaType = "application/json", schema = @Schema()))
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "아이디가 존재하지 않는 경우", content = @Content)
    public ResponseEntity<ApiResponse<String>> submitSeniorAgreement(@RequestBody @Valid SubmitAgreementsRequest request, Authentication authentication){
        authService.submitSeniorAgreement(authentication.getName(),request);
        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse.success("성공", SuccessStatus.SUCCESS_200));
    }
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "아이디 중복 확인 완료", content = @Content(mediaType = "application/json", schema = @Schema(implementation = CheckLoginIdDuplicateResponse.class)))
    ...
    public ResponseEntity<ApiResponse<ReissueAccessTokenResponse>> reissueAccessToken(@Parameter(hidden = true) @RequestHeader("Authorization") String refreshToken) {
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.success(response, SuccessStatus.SUCCESS_200));
    }
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "204", description = "비밀번호 변경 완료", content = @Content(mediaType = "application/json"))
    ...
    public ResponseEntity<ApiResponse<String>> changePassword(@RequestBody @Valid ChangePasswordRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.NO_CONTENT).body(ApiResponse.success("비밀번호 변경 완료", SuccessStatus.SUCCESS_204));
    }
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "인증번호 확인 완료", content = @Content(mediaType = "application/json", schema = @Schema(implementation = SmsVerifyConfirmResponse.class)))
